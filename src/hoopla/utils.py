...
def _load_prompt_enchance() -> dict[str, str]:
    prompts_path = DATA_DIR / "prompts" / "enhance_prompts.json"
    with open(prompts_path, mode="r", encoding="utf-8") as f:
        enhance_prompts = json.load(f)
    return enhance_prompts


def call_gemini(query: str, enhance: Optional[str] = None) -> str:
    load_dotenv()
    api_key = os.environ.get("GEMINI_API_KEY")
    client = genai.Client(api_key=api_key)

    prompts = _load_prompt_enchance()
    if enhance in prompts:
        query = prompts[enhance].format(query=query)
    ...
    try:
        ...
        print("Given prompt:", query)
        print("Enhance method:", enhance)
        sys.exit(1)
    if response is None or response.text is None:
        print("No response generated by gemini. Verify your API KEY")
        sys.exit(1)
